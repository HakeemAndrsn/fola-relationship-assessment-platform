...
function NewsletterSignup({ variant = "default" }: { variant?: "default" | "inline" }) {
  ...
  if (submitted) return (
    <div className="text-center py-4">
      <p className="text-[#B8654A] font-serif italic">Welcome to LOVEBetter.</p>
  ...
  if (variant === "inline") return (
    <form onSubmit={handleSubmit} className="flex gap-3 max-w-md mx-auto">
      <input type="email" required value={email} onChange={(e) => setEmail(e.target.value)} placeholder="your@email.com"
        className="flex-1 rounded-lg border border-border bg-white px-4 py-3 text-sm text-foreground placeholder-muted-foreground/60 focus:outline-none focus:border-accent/50 font-sans" />
      <button type="submit" className="bg-[#121212] text-[#F5F2EC] px-6 py-3 rounded-lg text-sm font-bold font-sans hover:bg-[#232323] transition-all whitespace-nowrap">
        Subscribe
      </button>
    </form>
  );
  return (
    <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row gap-3 max-w-lg mx-auto">
      <input type="email" required value={email} onChange={(e) => setEmail(e.target.value)} placeholder="Enter your email address"
        className="flex-1 rounded-xl border border-border bg-white px-5 py-4 text-sm text-foreground placeholder-muted-foreground/60 focus:outline-none focus:border-accent/50 font-sans" />
      <button type="submit" className="bg-[#121212] text-[#F5F2EC] px-8 py-4 rounded-xl text-sm font-bold font-sans hover:bg-[#232323] transition-all whitespace-nowrap">
        Join LOVEBetter
      </button>
    </form>
  );
...
export default function Home() {
  ...
  return (
    <div className={`min-h-screen bg-background text-card-foreground texture-paper transition-all duration-500 ${showSticky ? "pb-20" : "pb-0"}`} style={{ backgroundImage: "radial-gradient(ellipse 80% 60% at 50% -10%, rgba(184, 101, 74, 0.12) 0%, rgba(124, 134, 115, 0.06) 50%, transparent 70%)" }}>
      ...
      <div className={`fixed bottom-0 left-0 right-0 z-50 transition-all duration-500 ${showSticky ? "translate-y-0 opacity-100" : "translate-y-full opacity-0"}`}>
        <div className="mx-auto max-w-7xl px-4 py-3">
          <div className="bg-card/95 backdrop-blur-xl border border-border rounded-2xl px-5 py-3 flex items-center justify-between gap-4 shadow-xl">
            <p className="text-sm text-foreground font-sans hidden sm:block">
              <span className="text-[#B8654A] font-bold">Done the assessment?</span> Ready to do the work?
            </p>
            <a href="https://calendly.com/folasessions/discovery-call" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 bg-[#121212] text-[#F5F2EC] px-5 py-2.5 rounded-xl text-xs font-bold font-sans hover:bg-[#232323] transition-all whitespace-nowrap">
        ...
        <div className="mx-auto max-w-7xl px-6 py-4 flex items-center justify-between gap-6">
          <div className="flex items-center gap-3">
            <Link href="/" className="flex items-center gap-2">
              ...
              <div>
                <p className="text-sm font-bold text-foreground tracking-tight font-serif leading-none">LoveBetter</p>
          ...
          {/* Mobile */}
          <Link href="/assessment" className="md:hidden inline-flex items-center gap-1.5 bg-[#121212] text-[#F5F2EC] px-4 py-2 rounded-lg text-xs font-bold font-sans border border-border">
            Start →
          </Link>
        </div>
      </header>
      ...
      <main className="pt-24">
        ...
        <section className="relative px-6 pt-16 pb-8 overflow-hidden">
          ...
          <div className="relative mx-auto max-w-5xl text-center">
            ...
            <h1 className="text-5xl sm:text-6xl lg:text-7xl font-bold text-foreground leading-[1.05] tracking-tight font-serif">
              Your relationship starts<br />
              <span className="italic" style={{ color: "#B8654A" }}>with who you are.</span>
            </h1>
            ...
            <div className="mt-12 flex flex-col sm:flex-row items-center justify-center gap-4">
              <Link
                ...
                className="group w-full sm:w-auto flex items-center justify-between sm:justify-start gap-4 rounded-2xl border border-border bg-card hover:bg-[#E9E1D6]/40 px-6 py-5 transition-all shadow-sm"
              >
                <div className="text-left">
                  <p className="text-[10px] text-card-foreground/60 uppercase tracking-wider font-sans">Individual</p>
                  <p className="text-base font-bold text-foreground font-serif">Personal Growth Assessment</p>
                  <p className="text-xs text-[#7C8673] font-sans mt-0.5">10 dimensions · R600 · 20–30 min</p>
                </div>
                <svg className="w-5 h-5 text-card-foreground/60 group-hover:text-foreground group-hover:translate-x-1 transition-all shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M13 7l5 5m0 0l-5 5m5-5H6" />
                </svg>
              </Link>
              <Link
                href="/assessment"
                className="group w-full sm:w-auto flex items-center justify-between sm:justify-start gap-4 rounded-2xl border border-[#B8654A]/30 bg-[#B8654A]/5 hover:bg-[#B8654A]/10 px-6 py-5 transition-all shadow-sm"
              >
                <div className="text-left">
            ...
            <div className="mt-6 inline-flex items-center gap-2 px-4 py-2 rounded-full border border-green-600/20 bg-green-600/5 backdrop-blur-sm">
        ...
        <section className="px-6 py-16">
          <div className="mx-auto max-w-7xl">
            ...
            <div className="grid lg:grid-cols-2 gap-6">
              ...
              {/* Individual Card */}
              <div className="group relative rounded-3xl border border-border bg-card p-8 hover:border-border/80 hover:shadow-md transition-all overflow-hidden shadow-sm">
                <div className="absolute top-0 right-0 w-48 h-48 rounded-full opacity-35 blur-3xl" style={{ background: "radial-gradient(circle, rgba(124,134,115,0.15) 0%, transparent 70%)" }} />
                <div className="relative">
                  <div className="flex items-start justify-between mb-6">
                    <div>
                      <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-[#7C8673]/10 border border-[#7C8673]/20 mb-3">
                        <span className="w-1.5 h-1.5 rounded-full bg-[#7C8673]" />
                        <span className="text-[10px] text-[#7C8673] font-semibold uppercase tracking-wider font-sans">Individual</span>
                      </div>
                      <h3 className="text-2xl font-bold text-foreground font-serif">Personal Growth<br />Assessment</h3>
                    </div>
                    <div className="text-right">
                      <p className="text-2xl font-bold text-foreground font-serif">R600</p>
                  ...
                  <div className="space-y-2 mb-6">
                    {[
                      ...
                      <div key={i} className="flex items-start gap-2.5">
                        <svg className="w-3.5 h-3.5 text-[#7C8673] mt-0.5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
                  ...
                  <div className="flex items-center justify-between pt-4 border-t border-border">
                    ...
                    <Link href="/individual-assessment" className="flex items-center gap-2 bg-[#121212] text-[#F5F2EC] px-5 py-2.5 rounded-xl text-xs font-bold font-sans hover:bg-[#232323] transition-all">
              ...
              {/* Couples Card */}
              <div className="group relative rounded-3xl border border-border bg-card p-8 hover:border-border/80 hover:shadow-md transition-all overflow-hidden shadow-sm">
                <div className="absolute top-0 right-0 w-48 h-48 rounded-full opacity-40 blur-3xl" style={{ background: "radial-gradient(circle, rgba(184,101,74,0.15) 0%, transparent 70%)" }} />
                <div className="relative">
                  <div className="flex items-start justify-between mb-6">
                    <div>
                      <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-[#B8654A]/10 border border-[#B8654A]/20 mb-3">
                        <span className="w-1.5 h-1.5 rounded-full bg-[#B8654A]" />
                        <span className="text-[10px] text-[#B8654A] font-semibold uppercase tracking-wider font-sans">Couples</span>
                      </div>
                      <h3 className="text-2xl font-bold text-foreground font-serif">Relationship Growth<br />Assessment</h3>
                    </div>
                    <div className="text-right">
                      <p className="text-2xl font-bold text-[#B8654A] font-serif">R600</p>
                  ...
                  <div className="space-y-2 mb-6">
                    {[
                      ...
                      <div key={i} className="flex items-start gap-2.5">
                        <svg className="w-3.5 h-3.5 text-[#B8654A] mt-0.5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
                          <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
                        </svg>
                        <p className="text-xs text-card-foreground/80 font-sans leading-relaxed">{item}</p>
                      </div>
            ...
            <div className="grid sm:grid-cols-3 gap-6">
              {[
                ...
                <div key={item.step} className="relative rounded-2xl border border-border bg-card p-7 hover:border-border/80 transition-all shadow-sm">
                  <p className="text-4xl font-bold text-[#B8654A]/20 font-serif mb-4">{item.step}</p>
                  <h3 className="text-base font-bold text-foreground font-serif mb-2">{item.title}</h3>
          ...
          <div className="mx-auto max-w-4xl">
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {[
                ...
                <div key={item.title} className="rounded-2xl border border-border bg-card p-6 hover:border-[#B8654A]/30 transition-all group shadow-sm">
                  <div className="w-9 h-9 rounded-xl bg-[#B8654A]/10 border border-[#B8654A]/20 flex items-center justify-center mb-4 group-hover:bg-[#B8654A]/15 transition-all">
                    <svg className="w-4.5 h-4.5 text-[#B8654A]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                      <path strokeLinecap="round" strokeLinejoin="round" d={item.icon} />
                    </svg>
                  </div>
              ...
              {[
                {
                  ...
                  color: "border-border", bg: "bg-card", btn: "bg-[#121212] text-[#F5F2EC] hover:bg-[#232323]", tagColor: "text-[#7C8673] bg-[#7C8673]/10 border-[#7C8673]/20",
                },
                {
                  ...
                  color: "border-border", bg: "bg-card", btn: "bg-[#121212] text-[#F5F2EC] hover:bg-[#232323]", tagColor: "text-[#B8654A] bg-[#B8654A]/10 border-[#B8654A]/20",
                },
              ].map((item) => (
                <div key={item.tag} className={`rounded-2xl border ${item.color} ${item.bg} p-7 shadow-sm`}>
                  <div className="flex items-center justify-between mb-5">
                    ...
                    <p className="text-2xl font-bold text-foreground font-serif">{item.price}</p>
                  </div>
                  <ul className="space-y-2.5 mb-6">
                    {item.features.map((f) => (
                      <li key={f} className="flex items-center gap-2.5 text-xs text-card-foreground/80 font-sans">
                        <svg className="w-3.5 h-3.5 text-[#B8654A] shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
                          <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
                        </svg>
                        {f}
                      </li>
        ...
        <section className="px-6 py-8">
          <div className="mx-auto max-w-3xl">
            <div className="rounded-3xl border border-[#B8654A]/20 bg-gradient-to-br from-[#B8654A]/[0.06] to-transparent p-8 text-center relative overflow-hidden shadow-sm">
              ...
              <div className="relative">
                <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-[#B8654A]/10 border border-[#B8654A]/20 text-[10px] text-[#B8654A] font-semibold uppercase tracking-wider font-sans mb-4">
                  Best Value
                </span>
                <h2 className="text-2xl sm:text-3xl font-bold text-foreground font-serif mb-2">The Complete Growth Bundle</h2>
                <p className="text-sm text-card-foreground/80 max-w-lg mx-auto leading-relaxed font-sans mb-4">
                ...
                <div className="flex items-center justify-center gap-4 mb-6">
                  <p className="text-3xl font-bold text-foreground font-serif">R1,000</p>
        ...
        <section className="px-6 py-16">
          <div className="mx-auto max-w-4xl">
            <div className="rounded-3xl border border-border bg-card p-10 text-center shadow-sm">
              ...
              <Link href="/the-uncommon-practice" className="inline-flex items-center gap-2 rounded-xl border border-border bg-secondary/40 px-7 py-3.5 text-sm text-foreground font-sans font-semibold hover:bg-secondary transition-all">
                Read the essays
        ...
        <section className="px-6 py-16">
          <div className="mx-auto max-w-2xl">
            ...
            <div className="space-y-3">
              {faqs.map((faq, i) => (
                <div key={i} className={`rounded-2xl border transition-all ${openFaq === i ? "border-[#B8654A]/30 bg-[#B8654A]/5" : "border-border bg-card hover:border-border/80 shadow-sm"}`}>
                  <button onClick={() => setOpenFaq(openFaq === i ? null : i)} className="w-full flex items-center justify-between px-6 py-5 text-left gap-4">
                    <span className="text-sm font-semibold text-foreground font-sans">{faq.q}</span>
      ...
      <footer className="border-t border-border px-6 py-10">
        ...
        <div className="mx-auto max-w-7xl mt-6 pt-6 border-t border-border/40 text-center">
          <p className="text-[10px] text-card-foreground/60 font-sans tracking-[0.05em]">
            Made Possible By{" "}
            <Link href="/angels" className="text-[#B8654A] hover:text-[#B8654A]/80 transition-colors">
              FOLA Angels
            </Link>
            : Melitah Motlhale
          </p>
